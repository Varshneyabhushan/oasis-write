...
**Follow the `release` skill (`.claude/skills/release/SKILL.md`). It is the authoritative procedure — read it in full before touching a version number or a tag, and do not improvise from the summary below or from `RELEASE_CHECKLIST.md`.** It covers version bumping, the CHANGELOG, committing, tagging, pushing, and the CI build, including the parts that are easy to get wrong.
